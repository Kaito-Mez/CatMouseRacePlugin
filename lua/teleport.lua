...
local jumpEvent = ac.OnlineEvent({
  ...
  ac.debug("sender",tostring(sender))
  ac.debug("message",tostring(message))
  ac.debug("tarpos", tostring(message.targetPosition))
  ac.debug("tardir", tostring(message.targetDirection))
  ac.debug("tarid", tostring(message.targetSessionID))
  ac.debug("teststring", "bah")
  ac.debug("testint", 1)
...
ui.registerOnlineExtra(ui.Icons.FastForward, 'Teleport somebody', nil, teleportHUD, teleportHUDClosed, ui.OnlineExtraFlags.None)
